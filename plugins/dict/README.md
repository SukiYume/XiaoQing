...
直接输入词汇即可查询，需要收窄或翻页时添加选项。`--exact`、`--page` 和 `--size` 可位于查询词前后；`-e` 等价于 `--exact`，`-p` 等价于 `--page`，`-n`、`--num` 和 `--page-size` 等价于 `--size`。每页默认 10 条；结果会标明查询方向、总数、当前页和总页数，并给出可复制执行的上一页/下一页命令。翻页命令完整携带查询条件，不依赖用户或群会话状态。
