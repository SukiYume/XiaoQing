...
生成提示词集中描述身份、当前任务、证据确定性与表达偏好。checker 区分事实断言、明确假设和用户邀请的推测；本地风险信号仅决定是否需要语义审查。普通陈述不会因包含疑问词被自动删句，图片相关文字也不会强制触发固定告知话术。

生成与审查复用 `llm/media_evidence.py` 的图像证据说明，区分附件存在与画面成功解析。`image_evidence_respected` 同时检查事实来源、缺图说明及文字来源归因，直接附和未知画面的评价也参与判断。用户要求的配文、比喻、拟人和引用译文保持创作或引用性质。

`always` 模式遇到检查器不可用时停止发送未审查候选，明确请求收到简短失败提示，主动参与保持静默。`risk` 保留检查器不可用时的兼容降级策略。直接 checker API 通过 `severity=infra` 标识不可用，生成器在候选放行判断之前实施各模式策略。

checker 保留当前消息与传入对话资料，审查规则与 JSON 材料分为独立消息。输出先列出陈述与证据，再给出判断；视觉陈述单独核验原文出处。思考模式和强度沿用路由配置，默认 8192 token 预算供推理和判定共同使用。默认 `always` 对每个候选执行语义审查，审查总预算为 30 秒并透传至 AI 服务，单模型超时沿用路由配置，保留一次重生成，整轮还受插件回调预算约束。`risk` 可由部署者选择以减少远程调用，其路由可能漏过未识别的语义要求。独立质量验收的机器门禁覆盖响应、协议和持久化；无追问、缺图事实边界和话题相关性进入人工复核材料，语义结论单独记录。
...
`task_scheduler.py` 管理防抖保存、任务登记、异常记录与关闭等待。`shutdown()` 先停止接收任务并刷盘，随后取消后台任务并等待至多 2 秒。任务全部结束时执行最终刷盘；仍有任务存活时抛出 `TimeoutError`，由生命周期层记录关闭失败。Core 的总关闭预算由所有插件共享，后台学习和防抖等待直接进入取消收尾。

会话锁缓存和弱引用注册表共同维护锁身份。缓存淘汰后，持有者与等待者继续共享原锁。消息编号首次分配时读取已有历史的最大 `mN`，覆盖重启、闲置重置和缓存淘汰后的续写。

表达与黑话的加载结果携带独立基线，保存以该基线向最新磁盘状态合并增量。快照可跨异步任务及工作线程传递。表情收集和使用计数通过索引路径锁提交，耗时修复在锁外分析并按旧快照校验后提交差量；发生冲突的条目留待下一轮修复。

人物档案目录的 `.generation` 标记定义重置代际。重置在共享路径锁内更新标记、清理 JSON 和备份、删除向量记录。人物提取在远程调用前记录代际，提交时核对标记。所有面向聊天的记忆查询统一排除 `quarantined_` 类型。

审查状态读取依次验证主文件与备份的 JSON 语法和业务结构。恢复有效备份时直接替换主文件，保持备份字节；主副本均不可用时隔离损坏主文件。
