...
def _media_placeholder(index: int) -> str:
    return f"[[xc_media_{max(1, int(index))}]]"
...
def _resolve_media_items(
    ...
    *,
    ...
    return resolve_registered_media_items(normalized_items, store=store)
...
def insert_or_merge_media_part(
    ...
    media_face_id = str(normalized_media.get("face_id", "") or "").strip()
    media_hash = str(normalized_media.get("media_hash", "") or "").strip()

    for index, item in enumerate(parts):
        if str(item.get("kind", "") or "").strip() != media_kind:
            continue
        if media_face_id and str(item.get("face_id", "") or "").strip() != media_face_id:
            continue
        if media_hash and str(item.get("media_hash", "") or "").strip() != media_hash:
            continue
        merged = dict(item)
...
def template_with_fallback_placeholders(
    ...
    if not items:
        return normalized
    if _MEDIA_PLACEHOLDER_RE.search(normalized):
        return normalized
...
def build_message_parts_from_template(
    ...
    *,
    ...
    for match in _MEDIA_PLACEHOLDER_RE.finditer(normalized_template):
...
def build_message_parts(
    ...
    *,
    ...
    if _MEDIA_PLACEHOLDER_RE.search(raw_text):
        template = raw_text
    else:
        visible_text = rebuild_message_content(raw_text, resolved_items, resolved_items=resolved_items)
...
def render_message_parts(
    ...
    *,
    ...
    if not content and not media_items:
        return ""
    return resolve_message_content(content, media_items, store=store)
