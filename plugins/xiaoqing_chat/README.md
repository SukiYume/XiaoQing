...
`xiaoqing_chat` 为 XiaoQing 提供拟人聊天能力。插件结合群聊参与判断、近期上下文、长期记忆、人物资料、行为规划、多模态消息和回复检查，让配置的机器人角色以群友身份参与 QQ 对话。
...
会话重置会清理当前会话的人物档案、备份及向量记忆。重置前启动的人物提取任务会在提交时失效，后续新事实从空档案开始积累。自动闲置重置保留完整聊天历史，消息编号从已保存历史继续递增。

聊天历史预算优先保留最新消息。规划器保留本轮目标输入，合法的等待时长按 0–30 秒执行。表达自审允许无需修改的通过结果和直接拒绝结果。

明确要求本轮不追问时，语义检查结合完整请求判断候选是否遵守要求。引用、间接陈述、条件句和用户邀请的假设按各自含义处理。候选正文完整提交审查，确认违反交流要求后进入重生成或受控回退。

图片证据由结构化消息附件与历史媒体记录提供。具体视觉断言需要可靠摘要或用户描述支持；无法观察画面时可自然说明限制。图片相关的编程、概念讨论及已由文字给出的信息按实际问题回答。生成与审查共享图像证据状态，历史图片按对应摘要讨论。

默认 `always` 模式逐条审查候选，按配置使用检查模型的思考能力。检查器不可用时，明确请求收到简短失败提示，主动参与保持静默。管理员可选择 `risk` 减少远程检查；该模式保留检查器不可用时的兼容降级行为。

表情库并发收集与使用统计会串行提交。相似图复用已有素材时，描述、标签和内容哈希随代表图一起保留。
...
- 用户只喊 Bot 名称后继续追问。
- reply 引用机器人的消息。
- 最近上下文已锚定机器人角色，当前消息使用 `她`、`他` 或 `ta` 等共指召唤。

共指判定要求近期历史存在机器人角色锚点，普通代词由群聊上下文继续处理。
...
机器人名称由项目配置 `config/config.json` 中的 `bot_name` 唯一定义。该名称同时用于群聊召唤、提示词人格、规划器、回复检查、表达学习、历史角色标签和默认上线通知。修改后执行 `/reload` 即可让后续轮次使用新名称。

`plugins/xiaoqing_chat/config/xiaoqing_config.json` 中的 `personality.identity` 只描述人物特征与边界，`brain_chat.brain_identity` 只描述深度对话方式；这两个字段不填写角色姓名。
