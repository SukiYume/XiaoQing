...
from .llm.media_evidence import (
    event_has_current_image,
)
...
def _concise_public_identity(identity: str) -> str:
    ...
    # 配置使用第二人称描述机器人；公开简介的分句主语统一转为第一人称。
    lead = re.sub(r"(^|[。！？!?，,]\s*)你(?!们)", r"\1我", lead)
...
def _build_attempt_messages(
    ...
    messages = build_prompt_messages(
        ...
        current_image_attached=event_has_current_image(plan.event),
...
async def _check_candidate_draft(
    ...
    if (
        ...
        and str(getattr(plan.runtime.cfg.reply_check, "llm_checker_mode", "always")).strip().lower()
        == "always"
    ):
        check = ReplyCheckResult(
            suitable     = False,
    ...
    return _RejectedCandidate(
        ...
        allow_after_regen_exhausted = (
            check.failure_code != "checker_unavailable"
            and not check.is_hard
            and structural_rejection is None
            and media_rejection is None
        ),
    )
...
def _queue_reply_regeneration(
    ...
    *,
    ...
    if rejected.result.failure_code == "checker_unavailable":
        # 审查预算已经耗尽，重生成不会恢复检查能力。
        return False
...
def _finish_rejected_candidate(
    ...
    if plan.forced and rejected.result.failure_code == "media_grounding":
        fallback_texts = ("这部分画面细节我目前无法确认，可以补充图片或相关描述。",)
        fallback_step  = "reply.check.exhausted.media_grounding"
    elif plan.forced and rejected.result.failure_code == "persona_grounding":
        ...
        if bot_name and _KNOWN_NAME_QUERY_RE.search(str(plan.text or "")):
            ...
            fallback_texts = ("当前人设没有设定这些具体资料，我无法确认。",)
