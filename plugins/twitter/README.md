...
两个命令都支持 `help`、`帮助` 和 `?`。Manifest 每天 03:00 启动一次后台抓取，任务更新缓存并保持群聊静默。
...
API 请求使用配置的认证头、Cookie 和代理。媒体下载使用独立的最小请求头，并通过安全公网抓取器访问媒体域名。真实认证信息保存在 secret 配置与普通日志边界之外。
...
6. 单轮新增达到 100 张、分页达到上限或连续两页无新图时结束。
...
| 现象 | 检查项 |
| --- | --- |
| `/twimg` 提示缓存为空 | 由管理员执行 `/tw_fetch`，并检查完成通知 |
