...
插件按账号维护首次全量完成标记。新账号首次抓取会遍历允许的全部分页；全量完成后进入增量模式，连续两页没有新增图片时结束本轮抓取。
...
API 请求使用配置的认证头、Cookie 和代理。媒体下载沿用同一代理，使用独立的最小请求头，并限制为受信 Twitter 媒体域名、HTTPS 协议和有界响应。真实认证信息保存在 secret 配置与普通日志边界之外。
