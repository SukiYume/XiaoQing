...
class PendoException(Exception):
    ...
    def __init__(
        self, message: str, user_message: str | None = None, error_code: str | None = None
    ):
...
class DuplicateItemException(PendoException):
    ...
    def __init__(self, item_type: str, identifier: str):
        super().__init__(
            f"Duplicate {item_type}: {identifier}", f"❌ 已存在相同的{item_type}: {identifier}"
        )
...
class OwnershipException(PendoException):
    ...
    def __init__(self, item_id: str):
        super().__init__(
            f"User does not own item: {item_id}", f"❌ 你不是 {item_id} 的所有者，无法修改"
        )
...
class InvalidTimeFormatException(PendoException):
    ...
    def __init__(self, time_str: str, expected_format: str | None = None):
        format_hint = f"\n期望格式: {expected_format}" if expected_format else ""
        super().__init__(
            f"Invalid time format: {time_str}", f"❌ 时间格式不正确: {time_str}{format_hint}"
        )
...
class TimeConflictException(PendoException):
    ...
    def __init__(self, new_event: dict[str, Any], conflicting_events: list[dict[str, Any]]):
        conflict_list = "\n".join(
            [f"  • {e.get('title', '未命名')}" for e in conflicting_events[:3]]
        )
        super().__init__(
...
class InvalidDateRangeException(PendoException):
    ...
    def __init__(self, start: str, end: str):
        super().__init__(
            f"Invalid date range: {start} to {end}", f"❌ 日期范围无效: {start} 到 {end}"
        )
...
class NaturalLanguageParseException(PendoException):
    ...
    def __init__(self, input_text: str, reason: str | None = None):
        reason_msg = f": {reason}" if reason else ""
        super().__init__(
            f"Failed to parse: {input_text}{reason_msg}", f"❓ 无法理解你的输入{reason_msg}"
        )
...
class MissingRequiredFieldException(PendoException):
    ...
    def __init__(self, field_name: str, field_desc: str | None = None):
...
class InvalidFieldValueException(PendoException):
    ...
    def __init__(self, field_name: str, value: str, valid_values: list[str] | None = None):
...
class TooManyResultsException(SearchException):
    ...
    def __init__(self, count: int, max_count: int):
        super().__init__(
            f"Too many results: {count}", f"⚠️ 找到 {count} 个结果，请使用更具体的关键词"
        )
...
class ExportException(PendoException):
    ...
    def __init__(self, format_type: str, details: str | None = None):
...
class ConfigurationException(PendoException):
    ...
    def __init__(self, config_key: str, details: str | None = None):
