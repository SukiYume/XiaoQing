...
def build_ledger_insights(
    ...
    focus_direction = direction if direction in {"expense", "income"} else "expense"
    ...
    if focus_start and focus_end:
        ...
        trend_rows = conn.execute(
            f"SELECT ledger_date, amount, created_at, id FROM items WHERE {' AND '.join(focus_where)} ORDER BY ledger_date, created_at, id",
            focus_params,
        ).fetchall()
        ...
        focus_total = totals_by_direction.get(focus_direction, 0.0)
        categories = [{
    ...
    focus_total = round(totals_by_direction.get(focus_direction, 0.0), 2)
    focus_count = counts_by_direction.get(focus_direction, 0)
