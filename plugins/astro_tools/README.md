...
单位首先按 Astropy 的标准大小写解析，标准解析失败后再尝试内置别名。`MJy` 与 `mJy`、`MK` 与 `mK`、`Mpc` 与 `mpc` 保持各自前缀含义。

`formula calc luminosity` 使用太阳单位下的分段主序星近似：质量低于 0.43 时为 `0.23 M^2.3`，0.43 至 2 时为 `M^4`，2 至 20 时为 `1.5 M^3.5`，20 及以上为 `2500 M`。分段边界采用右侧区间，结果用于经验估算；`formula calc lifetime` 使用独立的寿命近似。
