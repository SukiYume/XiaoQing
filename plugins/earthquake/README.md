...
`earthquake` 从[中国地震台网速报微博](https://www.weibo.com/ceic)读取近期地震快讯。手动命令查询最新记录，定时任务向默认群投递 4.0 级及以上的新快讯。
...
插件凭据配置为空集。手动查询需要访问微博与新浪图片域名；自动投递需要至少一个 Core 默认群。
