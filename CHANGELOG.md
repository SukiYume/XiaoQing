...
- 移除 Pendo 账本页的币种筛选框，恢复原有筛选布局，默认使用人民币。
- 清理对应事件监听和可变状态，账本与币种相关的 11 项回归测试通过。
...
- 将 Shell 继承管道的读取纳入原命令预算，父进程先退出时仍按时回收子进程；限制 Coze 请求预算的浮点上溢，并使用显式同步验证线程迟到异常。
- 更正 checker 路由配置路径为 `plugins.xiaoqing_chat.ai.routes.checker`。
- 修复格式检查器报告显式相对路径与仓库外文件时的路径异常，检查模式保持文件内容不变。
...
- 缩短回复和 checker 提示词，区分事实、明确假设及用户邀请的推测；完整保留本轮审查证据。
- 删除疑问词驱动的自动删句与缺图固定措辞门禁，确认语义违规后使用重生成流程。
- 默认逐候选执行语义检查，移除请求中强制关闭思考的配置覆盖，使用 8192 token 输出预算、30 秒审查预算及一次重生成上限；总预算透传，单模型超时沿用路由配置，为后备模型保留机会。
- 质量脚本移除生产判定函数依赖，机器指标和独立人工语义结论分别记录。
- 缺图和解析失败时明确说明画面不可用，视觉评价核对文字来源；保留文字创作、翻译和编程任务。checker 路由支持通过 `request_defaults.thinking.type=disabled` 关闭思考。
- 将维护约定并入插件开发指南，清理临时审查文档与文件清单。

### normal 环境真实运行验收

- 使用 `conda activate normal` 启动 `python main.py`，验证全部 30 个插件加载、HTTP/WebSocket 命令矩阵、Pendo API、真实模型回复和压力恢复。
- 压力工具添加动作预览响应头，验收请求直接读取生成的动作；真实 QQ 收发由生产环境负责。
- 修复聊天公开身份的第一人称表达和后台任务关闭预算，进程入口对资源关闭失败返回非零退出状态。
- 生成与审查使用交流约束和实际图像证据，验收独立记录语义结果；用户提及截图且缺少可用图像时，回复明确说明画面未知，历史真实图片保留独立证据边界。
- 按实际业务响应记录上游服务和本机网络限制，生产服务的可用性由实际运行监测。
- 发布前 normal 回归 6,272 项通过、2 项 POSIX 平台跳过，覆盖率 81.09%；815 个代码文件格式、373 个生产文件类型、文档链接及发行包构建检查通过。

### 完整审查修复与代码维护

- 修复标准 HTTP 上报的实际回复通道、投递收据计数与 WebSocket 连接通知生命周期，并明确入站 Bearer 与出站凭据配置。
- 修复全部插件审查确认的持久化、取消回收、资源边界、时间与币种、导入导出、聊天记忆和前端并发编辑问题。
- 将最低 Python 版本统一为 3.11，清理失效兼容分支与无用导入，补充关键边界的中文注释。
- 新增统一代码格式器，Python 转换校验 AST，JavaScript 与 PowerShell 对齐校验非空白 token；CI 使用同一格式检查入口。
- 同步项目手册、插件 README 和架构文档。原始审查确认的 58 项问题已完成修复与对应回归。
- 审查修复阶段验证：6,220 项测试通过、9 项环境相关跳过，综合覆盖率 81.32%；后续发布前验证结果见上文。

### Pendo 日程编辑结果摘要

- 单次日程编辑成功后展示实际落库结果的字段差异；时间差异包含完整日期、中文星期、时刻和日程 IANA 时区。
- 日程时间带动提醒重算时，回复同时展示提醒同步调整结果；标题、地点、分类、标签、内容和备注编辑使用紧凑的修改前后对照。
- 回归覆盖“下周三晚上 8 点”编辑结果、上海时区 UTC 入库和三条提醒同步重算。
- 验证：Pendo 完整测试集 1140 项通过，相关 Ruff 格式、Ruff 检查、369 个生产模块的 Mypy 检查和 `git diff --check` 通过。

### Pendo Web 用户时区统一

- Web 路由在页面装载前读取当前用户持久化的 IANA 时区；总览、日程、待办、笔记、日记、搜索、统计与迁移记录统一按该时区显示时间、判断当天并推导自然日期范围。
- 带偏移时间按真实时刻转换，旧无偏移时间继续解释为用户墙钟，纯日期字段保持 `YYYY-MM-DD` 语义；UTC 持久化格式保持不变，现有时间字段无需迁移。
- 总览日程使用服务端返回的真实时间轴毫秒值判断进行中与已结束状态，日记近期时间和自动标题按用户时区生成；设置保存后立即刷新前端时区运行态。
- 移除通用格式模块中的浏览器本地时区展示接口，迁移导出清单与操作记录也使用当前用户时区。
- 验证：Pendo 完整测试集 1139 项通过，相关 Ruff 格式、Ruff 检查、Mypy 检查与 `git diff --check` 通过。

### Pendo iOS 日历对账

- Scriptable 日历同步按完整 Pendo UUID 原地更新已有事件的标题、起止时间、全天状态和地点，QQ 或 Web 修改日程后不再保留旧时间。
- 同步窗口始终至少回看 30 天；仅在服务端完整窗口内清理带 Pendo 标记和 ID 的已删除、已移出或重复事件，用户自建事件与无 ID 旧事件保持不动。
- 同步通知区分新增、更新、删除、未变化与跳过数量；任何日历写入失败时不推进成功游标，后续运行继续对账。
- Widget 客户端将日程解析、目标日历解析、现有事件索引、单条对账、失效清理和结果格式化拆分为独立边界，删除重复的备注解析与异步包装逻辑。
- 验证：Pendo 完整测试集 1139 项通过，相关 Ruff 检查与 `git diff --check` 通过。
...
- 逐份复核根目录、文档目录和 30 个插件共 32 份 README，以及 `docs/` 下的专题手册和 Pendo 架构文档，统一使用直接描述现行功能、配置、边界和扩展流程的独立文档结构。
- README 只保留当前能力、使用方法和运行契约；自动化门禁拒绝更新日志、版本或日期标题及“本次更新”式叙述，版本变化统一进入 Changelog。
