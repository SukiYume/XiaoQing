...
- `scripts/sync_to_remote.sh` 不再排除生产实际使用的 `plugins/arxiv_filter/best_model/`；模型配置、权重、tokenizer 和训练配置成为同步前必检、同步后复核的发布资源，避免重新整理生产目录后只传代码而遗漏权重。
- 远端主机和目标目录集中放在脚本顶部，默认使用 `secondary-production-host:/c/Users/testuser/Desktop/XiaoQing/XiaoQing_V3`，需要时直接把主机改为 `production-host`，不再通过命令行参数或环境变量传递目标。
- 同步统一复用 `.gitignore` 排除本地缓存和训练产物，同时明确保护生产 `config/config.json`、`config/secrets.json`、Minecraft 连接配置、日志、数据库、插件缓存、备份和导出；增加 checksum 比对、延迟替换、断点保留和同步后的远端必需文件检查。
- 脚本继续默认 dry-run，实际写入仍要求 `--apply --confirm-delete`，且不会自动停止或启动生产进程；部署文档补充完整的停服、预览、应用、校验和启动顺序。

### 手机端分层命令帮助

- 统一 `/help` 文本目录改为渐进式下钻：总览每页显示少量插件，`/help <插件>` 只列一级入口，`/help <命令路径>` 只列直接子命令，叶节点详情才展示完整用法、别名、权限、场景和正反例；所有插件自动使用同一格式。
- 文本菜单与搜索结果不再为每个节点重复显示稳定 code、权限和示例，长用法及示例按约 34 个手机显示宽度在参数边界换行；去除会在窄屏折成两行的长分隔线，并提供明确的继续查看、上下页和返回入口。
- `/help json [查询] [page N]` 的完整字段、扁平目录和原有分页大小保持不变，UAT 命令矩阵及其他自动化消费者无需修改；`bot_core` 插件版本升至 `0.4.0`。

### Minecraft 消息帮助补全

- `/mc help` 现在明确展示 `/mc say <消息>` 全服广播和 `/mc tell <玩家名> <消息>` 定向私信，并说明日志监控启用后玩家聊天、加入和离开等事件会转发到发起连接的 QQ 私聊。
- 修正文档中会被 Minecraft 视为未知命令的 `/mc 大家好` 示例，统一改为 `/mc say 大家好`，同时补充 `say`、`tell`、`tellraw` 均为原样通过 RCON 执行的服务器命令。
...
- 修复 Linux 并行发布门禁中监控器清理测试读取到刚创建但尚未写完的 PID 标记文件的竞态；测试子进程现在通过同目录临时文件和原子替换发布 PID。
...
- `/help <插件名>` 继续展示该插件的完整递归命令目录，稳定命令码精确查询、关键词搜索、详情分页与上下页提示保持可用。
- `/help json [查询] [page N]` 保持原有的全量扁平命令导出契约，不影响 UAT 命令矩阵和其他自动化消费者。`bot_core` 插件版本升至 `0.3.0`。
