...
def shell():
    ...
    return results.ping, results.download,results.upload

def testmain():
    try:
        ping, download, upload = shell()
    ...
    return 'Ping: '+str(ping)+' ms \nDownload: '+str(np.round(download/10**6,2))+' Mbit/s \nUpload: '+str(np.round(upload/10**6,2))+' Mbit/s'
