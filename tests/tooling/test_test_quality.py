...
ALLOWED_PROJECT_SOURCE_READERS = {
    # 日志脱敏与敏感信息审计。
    (
        "plugins/contracts/test_plugin_crosscutting_boundaries.py",
        "test_sensitive_log_regressions_are_absent",
    ),
    (
        "plugins/contracts/test_internal_log_redaction.py",
        "test_targeted_exception_log_calls_never_receive_raw_exception_values",
    ),
    (
        "plugins/contracts/test_public_error_redaction.py",
        "test_public_plugin_runtime_never_uses_unredacted_traceback_logging",
    ),
    (
        "plugins/contracts/test_sensitive_logging_audit.py",
        "test_qingssh_and_minecraft_logger_calls_reject_sensitive_ast_arguments",
    ),
    (
        "plugins/contracts/test_shell_jupyter_log_privacy.py",
    ...
    (
        "plugins/xiaoqing_chat/test_xiaoqing_log_privacy.py",
    ...
    (
        "plugins/arxiv_filter/test_arxiv_bounded_http.py",
        "test_sync_arxiv_paths_forbid_unbounded_requests_and_response_access",
    ),
    (
        "plugins/contracts/test_configured_http_clients.py",
        "test_configured_clients_forbid_direct_response_body_access",
    ),
    (
        "plugins/contracts/test_fixed_origin_http_clients.py",
        "test_fixed_http_plugins_have_no_direct_response_body_reads",
    ),
    (
        "plugins/contracts/test_remaining_http_clients.py",
        "test_remaining_runtime_paths_have_no_direct_response_body_reads",
    ),
    (
        "plugins/astro_tools/test_simbad_bounded_http.py",
        "test_simbad_source_has_local_query_builder_and_bounded_transport",
    ),
    (
        "plugins/earthquake/test_earthquake.py",
        "test_runtime_has_no_direct_unbounded_response_reads",
    ),
    (
        "tooling/test_bounded_http_adoption.py",
        "test_runtime_http_paths_have_no_direct_unbounded_response_reads",
    ),
    # 共享参数、内容边界与配置快照的采用门禁。
    ("tooling/test_args_adoption.py", "test_reviewed_integer_consumers_use_core_parser"),
    ("tooling/test_args_adoption.py", "test_reviewed_plugins_use_core_argument_layer"),
    (
        "tooling/test_external_content_adoption.py",
    ...
    (
        "tooling/test_settings_snapshot_adoption.py",
    ...
    (
        "tooling/test_core_module_boundaries.py",
...
RETIRED_TEST_BUCKET_NAMES = {
    "test_cross_plugin_regressions.py",
    "test_pendo_fixes.py",
    "test_pendo_review_fixes.py",
    "test_qingpet_improvements.py",
    "test_qingpet_new_features.py",
    "test_qingpet_regressions.py",
    "test_xiaoqing_chat_refactor_cleanup.py",
    "test_xiaoqing_chat_review_regressions.py",
    "test_xiaoqing_review_fixes.py",
}
...
def _source_flags(
    ...
    if isinstance(node, ast.Name) and node.id == "LOG_PUMP":
        has_python_path = True
    # 目录重排后的测试统一从 ``tests.helpers.paths`` 引入这些常量；它们与
    # 旧的 ``Path(__file__)`` 推导具有相同的仓库锚定语义。
    project_anchored = isinstance(node, ast.Name) and node.id in {
        "__file__",
        "REPOSITORY_ROOT",
        "TESTS_ROOT",
        "PLUGINS_ROOT",
...
def test_test_modules_remain_bounded_and_semantically_named() -> None:
    ...
    for path in sorted(TESTS_ROOT.rglob("test_*.py")):
        ...
        if path.name in RETIRED_TEST_BUCKET_NAMES:
            violations.append(f"{relative_path}: replace historical bucket with a behavior owner")
        if relative_path.parent in {Path("."), Path("plugins")}:
            violations.append(f"{relative_path}: move test module into a responsibility directory")
        if path.name != "paths.py" and REPOSITORY_DEPTH_EXPRESSION.search(source):
            violations.append(f"{relative_path}: use tests.helpers.paths for repository roots")
