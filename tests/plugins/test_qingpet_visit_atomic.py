...
def test_visit_same_reference_returns_committed_grants_without_double_write(tmp_path):
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM asset_ledger WHERE reason LIKE 'pet_visit_%'"
        ).fetchone()[0]
        == 2
    )
...
def test_visit_two_connections_cannot_exceed_daily_quota(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(
            pool.map(lambda pair: run(*pair), [(first_db, "visit:a"), (second_db, "visit:b")])
        )

    assert sum(result.success for result in results) == 1
    assert (
        first_db._get_connection().execute("SELECT action_count FROM action_quotas").fetchone()[0]
        == 1
    )
...
def test_visit_clips_each_users_reward_independently_and_reports_actual_grants(tmp_path):
    ...
    ok, message = SocialService(db).visit_pet("visitor", "owner", GROUP, message_id="near-cap")

    assert ok is True
    assert "访客获得1金币" in message
    assert "宠物主人获得0金币" in message
    rows = conn.execute(
        "SELECT user_id, delta FROM asset_ledger WHERE reason LIKE 'pet_visit_%' ORDER BY user_id"
    ).fetchall()
...
def test_visit_reference_is_isolated_by_group(tmp_path):
    ...
    refs = (
        db._get_connection()
        .execute("SELECT reference_id FROM visit_settlements ORDER BY group_id")
        .fetchall()
    )
