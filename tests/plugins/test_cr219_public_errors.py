...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(choice, "parse", id="choice"),
        pytest.param(color, "parse", id="color"),
        pytest.param(dictionary, "parse", id="dict"),
        pytest.param(earthquake, "parse", id="earthquake"),
        pytest.param(echo, "segments", id="echo"),
        pytest.param(github, "parse", id="github"),
        pytest.param(guess_number, "parse", id="guess"),
        pytest.param(twitter, "parse", id="twitter"),
        pytest.param(chat, "parse", id="chat"),
        pytest.param(signin, "parse", id="signin"),
        pytest.param(smalltalk, "parse", id="smalltalk"),
        pytest.param(voice, "parse", id="voice"),
        pytest.param(wolframalpha, "parse", id="wolframalpha"),
    ],
)
...
async def test_remaining_public_plugins_share_the_same_safe_boundary(
    ...
    if plugin_name == "ads_paper":
        ...
        monkeypatch.setattr(ads_paper, "parse", Mock(side_effect=error))
        ...
        monkeypatch.setattr(xiaoqing_chat, "parse", Mock(side_effect=error))
...
def test_pendo_reminder_terminal_failure_propagates_without_raw_log(
    ...
    db = SimpleNamespace(
        get_all_events_with_reminders=Mock(side_effect=RuntimeError(SENSITIVE_ERROR))
    )
...
def test_pendo_recoverable_internal_failures_log_only_error_type(
    ...
    with caplog.at_level(logging.WARNING):
        ...
        assert resolve_default_category(settings_db, "user-1", "fallback") == "fallback"
...
def test_pendo_setting_write_failure_preserves_exception_without_duplicate_log(
    ...
    with caplog.at_level(logging.ERROR), pytest.raises(RuntimeError, match=CANARY):
        save_user_setting("user-1", "privacy_mode", True, settings_db)
...
async def test_pendo_ai_and_llm_degradation_never_logs_raw_exception(
    ...
    monkeypatch.setattr(
        "plugins.pendo.services.llm_client.chat_completions_with_fallback_paths",
        AsyncMock(side_effect=RuntimeError(SENSITIVE_ERROR)),
    )
...
def test_pendo_transfer_validation_and_constraint_detection_ignore_exception_text(
    ...
    _record, warning = pendo_transfer._validate_export_record({"_type": "event", "id": "event-1"})
