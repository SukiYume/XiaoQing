...
def test_smalltalk_qa_is_scoped_bounded_and_audited(tmp_path: Path) -> None:
    first = _ScopedContext(tmp_path, user_id=10, group_id=100)
    second = _ScopedContext(tmp_path, user_id=20, group_id=200)

    assert "成功" in str(smalltalk._add_qa(first, "问 回答一"))
    assert smalltalk.get_qa_answer(first, "问") == "回答一"
    assert smalltalk.get_qa_answer(second, "问") is None
    assert "不能超过" in str(
        smalltalk._add_qa(first, f"{'问' * (smalltalk.MAX_QUESTION_LENGTH + 1)} 回答")
    )
...
def test_apod_cache_key_uses_full_url_and_trusted_mime() -> None:
    one = apod._cache_filename("https://apod.nasa.gov/a/shared", "image/png")
    two = apod._cache_filename("https://apod.nasa.gov/b/shared", "image/png")
    assert one != two
...
def test_custom_colors_are_scoped_and_atomic(tmp_path: Path) -> None:
    ...
    color_data.mutate_custom_colors(first, lambda colors: colors.append({"name": "甲"}))
    color_data.mutate_custom_colors(second, lambda colors: colors.append({"name": "乙"}))

    assert color_data.load_custom_colors(first) == [{"name": "甲"}]
    assert color_data.load_custom_colors(second) == [{"name": "乙"}]
