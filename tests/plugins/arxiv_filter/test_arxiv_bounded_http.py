...
def test_runtime_html_uses_streamed_wrapper_and_rejects_wire_overflow(
    ...
    assert isinstance(timeout, requests.adapters.TimeoutSauce)
    for budget in (timeout.total, timeout.connect_timeout, timeout.read_timeout):
...
def test_update_date_uses_explicit_english_month_mapping(
    monkeypatch: pytest.MonkeyPatch,
    load_with_feedparser,
) -> None:
    module = load_with_feedparser(_ARXIV_TODAY, lambda _body: None)
...
def test_disabled_proxy_tls_verification_is_explicitly_logged(
    ...
    with caplog.at_level("WARNING", logger=module.logger.name):
        proxies, verify, _timeout = module._get_request_params(
            {"arxiv": {"proxy": "http://proxy.invalid", "use_ssl_verify": False}}
        )

    assert proxies == {"http": "http://proxy.invalid", "https": "http://proxy.invalid"}
...
def test_step2_resume_preserves_total_results_and_incomplete_state(
    monkeypatch: pytest.MonkeyPatch,
    load_with_feedparser,
) -> None:
    module = load_with_feedparser(
        _STEP2,
        lambda _body: _AttrDict(feed=_AttrDict(), entries=[]),
    )
    monkeypatch.setattr(
        module,
        "requests_request_bounded",
        lambda *_args, **_kwargs: _bounded_response(_VALID_ATOM),
    )
    previous = [{"arxiv_id": "2607.00001", "title": "t", "abstract": "a"}]
    ...
    assert result.total_results == 4000
    assert result.papers == previous
