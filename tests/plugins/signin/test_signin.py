...
from tests.helpers.http import QueuedAiohttpSession, aiohttp_json_response
...
class _Context:
    ...
    def queue(self, *payloads: object) -> None:
        self.raw_session.queue(*(aiohttp_json_response(payload) for payload in payloads))
...
async def test_unknown_platform_returns_help(
    ...
    assert context.raw_session.requests == []
...
async def test_all_yingshi_aliases_execute_same_flow(
    ...
    assert len(context.raw_session.requests) == 2
...
async def test_query_response_requires_json_object(context: _Context) -> None:
    ...
    assert len(context.raw_session.requests) == 1
...
async def test_token_uses_authorization_header_not_query(context: _Context) -> None:
    ...
    assert len(context.raw_session.requests) == 2
    for _method, _url, request_kwargs in context.raw_session.requests:
        assert request_kwargs["headers"]["Authorization"] == "Bearer test_access_token"
        assert "access_token" not in request_kwargs["params"]
