...
`/reload` 先确认配置 revision 已发布和插件后台重载已启动，任务结束后向原管理员会话发送一次成功或失败结果及耗时。静音默认 10 分钟，支持分钟、小时和中文单位，范围上限为 24 小时。Secret 路径使用点分命名，写入后发布新的配置 revision；查询结果按值类型脱敏。运行指标由 Core 观测服务提供。
