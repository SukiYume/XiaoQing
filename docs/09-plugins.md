...
| `/plugins` | 公开 | 查看已加载插件 |
| `/reload` | Bot 管理员 | 重载配置与插件，完成后通知结果与耗时 |
...
`/reload` 先确认配置 revision 已发布和插件后台重载已启动，任务结束后向原管理员会话发送一次成功或失败结果及耗时。静音默认 10 分钟，支持分钟、小时和中文单位，范围上限为 24 小时。Secret 路径使用点分命名，写入后发布新的配置 revision；查询结果按值类型脱敏。完整有效的 secrets 单文件候选会通过管理员私聊报告字段路径，核对后由 `/reload` 确认。运行指标由 Core 观测服务提供。
...
账本汇总按币种分别计算，Web 统计默认选择 CNY，并支持查询指定币种与读取 `by_currency` 分组；聊天摘要、导出和小组件保留币种标签。日程和待办输入按照用户时区解释，带偏移时间保存真实时刻，提醒队列统一使用 UTC。完整编辑提交读取时的条目版本，冲突时保留用户编辑并提示刷新。
...
软删除保留提醒历史，撤销删除会恢复仍需执行的提醒。重复规则的 `COUNT` 约束整个原始序列；查询窗口内的生成保持同一计数起点。备份采用 JSON 记录边界读取，中文文件名通过 UTF-8 百分号编码传递；导出规模与导入校验限制保持一致。
...
Scriptable 主屏摘要最多展示 5 条日程；脚本顶部配置 Web 地址和 Widget Token。直接运行脚本时，独立日历接口返回过去至少 30 天至未来 30 天的完整窗口；若成功日游标更早则同时补齐间隔。同步采用一次接口查询、一次目标日历查询，并按 Pendo 条目 ID 原地新增、更新和清理托管事件。
