# Pendo Ledger (记账) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a ledger/accounting command to the pendo plugin that supports interactive multi-turn entry, quick entry, listing, editing, deleting, and monthly summary statistics.

**Architecture:** Follows existing pendo plugin patterns — `LedgerHandler(DbOpsMixin)` for business logic, multi-turn session via `safe_create_session` for interactive add, unified `items` table with new columns for ledger-specific fields. No AI parsing needed (rule-based, like task/note).

**Tech Stack:** Python 3.12+, SQLite (existing pendo.db), dataclasses, async/await
...
|------|--------|----------------|
| `plugins/pendo/models/item.py` | Modify | Add `ItemType.LEDGER`, `LedgerItem` dataclass |
| `plugins/pendo/models/constants.py` | Modify | Add Ledger field constants |
| `plugins/pendo/config.py` | Modify | Add ledger categories, payment methods, session type |
| `plugins/pendo/services/db.py` | Modify | Add ledger columns via ALTER TABLE migration, register in `ITEM_TYPE_CLASS_MAP` import |
| `plugins/pendo/handlers/ledger.py` | Create | `LedgerHandler` — full CRUD + summary + session start |
| `plugins/pendo/commands/session.py` | Modify | Add `handle_ledger_add_session()` routing |
| `plugins/pendo/core/types.py` | Modify | Add `ledger_handler` to `PendoServices` TypedDict |
| `plugins/pendo/main.py` | Modify | Register handler, route, help text |
| `plugins/pendo/utils/formatters.py` | Modify | Add ledger type icon/name |

---

### Task 1: Data Model — `ItemType.LEDGER` + `LedgerItem`

**Files:**
- Modify: `plugins/pendo/models/item.py`
- Modify: `plugins/pendo/models/constants.py`

- [ ] **Step 1: Add `LEDGER` to `ItemType` enum**

In `plugins/pendo/models/item.py`, add to the `ItemType` enum:

```python
LEDGER = "ledger"  # 记账
```

- [ ] **Step 2: Add `LedgerItem` dataclass**

After `DiaryItem` class, add:
...
- [ ] **Step 3: Register in `ITEM_TYPE_CLASS_MAP`**
...
- [ ] **Step 4: Add field constants**

In `plugins/pendo/models/constants.py`, add after the Diary section:
...
```bash
git add plugins/pendo/models/item.py plugins/pendo/models/constants.py
git commit -m "feat(pendo): add LedgerItem data model and field constants"
```

---

### Task 2: Configuration — Categories, Payment Methods, Session Type

**Files:**
- Modify: `plugins/pendo/config.py`

- [ ] **Step 1: Add session type constant**

In `PendoConfig` class, after `SESSION_TYPE_EVENT_INFO`:

```python
    SESSION_TYPE_LEDGER_ADD = "ledger_add"
```

- [ ] **Step 2: Add ledger category and payment configs**

After `MOOD_ANALYSIS_CONFIG` at module level, add:
...
```bash
git add plugins/pendo/config.py
git commit -m "feat(pendo): add ledger configuration - categories, payment methods, session type"
```
...
**Files:**
- Modify: `plugins/pendo/services/db.py`

- [ ] **Step 1: Add ALTER TABLE migrations for ledger columns**

In `_init_database()`, add to the `migrations` list:
...
- [ ] **Step 2: Add `LedgerItem` to the import at top of db.py**
...
- [ ] **Step 3: Add `ledger_date` to `ALLOWED_DATE_FIELDS`**
...
```bash
git add plugins/pendo/services/db.py
git commit -m "feat(pendo): add ledger columns to database schema"
```

---

### Task 4: Formatter Updates

**Files:**
- Modify: `plugins/pendo/utils/formatters.py`

- [ ] **Step 1: Add ledger to TYPE_ICONS and TYPE_NAMES**
...
```bash
git add plugins/pendo/utils/formatters.py
git commit -m "feat(pendo): add ledger type icon and name to formatters"
```

---

### Task 5: LedgerHandler — Core CRUD + Summary

**Files:**
- Create: `plugins/pendo/handlers/ledger.py`

- [ ] **Step 1: Create `handlers/ledger.py` with full implementation**
...
class LedgerHandler(DbOpsMixin):
    ...
    async def summary(
        ...
        格式:
        - /pendo ledger summary          -> 本月
        - /pendo ledger summary 2026-03  -> 指定月份
        - /pendo ledger summary week     -> 本周
        """
...
```bash
git add plugins/pendo/handlers/ledger.py
git commit -m "feat(pendo): add LedgerHandler with CRUD, interactive add, and summary"
```

---

### Task 6: Session Routing — Wire Up Ledger Session

**Files:**
- Modify: `plugins/pendo/commands/session.py`

- [ ] **Step 1: Add ledger session routing**

In `handle_session_message()`, add a new condition before the fallback error return:
...
```bash
git add plugins/pendo/commands/session.py
git commit -m "feat(pendo): add ledger session routing for interactive add"
```

---

### Task 7: Type Registration + Main Wiring

**Files:**
- Modify: `plugins/pendo/core/types.py`
- Modify: `plugins/pendo/main.py`

- [ ] **Step 1: Add `ledger_handler` to `PendoServices`**

In `plugins/pendo/core/types.py`:
...
- [ ] **Step 2: Import and register LedgerHandler in main.py**
...
In `_get_services()`, after `search_handler = SearchHandler(db)`:
...
- [ ] **Step 3: Register command in `_build_command_router()`**

After getting `search_handler` from services, add:
...
In `HELP_MAP`, add a new `"ledger"` section:
...
In the `sections` list (within `_show_help`), add `"ledger"` after `"diary"`:
...
In the `aliases` dict (within `_show_help`), add:
...
In the `"quick"` section of HELP_MAP, add:
...
```bash
git add plugins/pendo/core/types.py plugins/pendo/main.py
git commit -m "feat(pendo): wire up ledger handler, routing, and help text"
```
...
- [ ] **Step 1: Verify Python can import the module without errors**

```bash
cd plugins/pendo && python -c "from handlers.ledger import LedgerHandler; print('OK')"
```
...
from plugins.pendo.models.item import LedgerItem, ItemType, ITEM_TYPE_CLASS_MAP
assert ItemType.LEDGER.value == 'ledger'
assert ItemType.LEDGER in ITEM_TYPE_CLASS_MAP
print('Model OK')

from plugins.pendo.config import LEDGER_EXPENSE_CATEGORIES, LEDGER_INCOME_CATEGORIES, LEDGER_PAYMENT_METHODS
assert len(LEDGER_EXPENSE_CATEGORIES) == 14
assert len(LEDGER_INCOME_CATEGORIES) == 6
assert len(LEDGER_PAYMENT_METHODS) == 6
print('Config OK')
"
```
