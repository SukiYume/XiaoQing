...
> [!TIP]
> 想快速定位问题？跳转 [9️⃣ 完整处理流程图](#9️⃣-完整处理流程图) 查看端到端示意图，再按需查阅对应章节。
...
- **`@` / 叫名字属于强制回复** - 进入插件后，被 `@` 或直接叫 `bot_name` 会标记为 forced，跳过普通回复概率判断
...
- **`@` / 叫名字会强制回复** - 一旦进入插件，被 `@` 或直接叫 `bot_name` 会走 forced 路径
